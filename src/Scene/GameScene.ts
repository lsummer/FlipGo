class GameScene extends egret.Sprite {
    ...
    // ----- 设定值-------
    private intervel: number = 3;       // 盒子间隔数
    ...
    private timeCount: number = SceneMange.timecount;
    ...
    private awardLeftCount: egret.TextField;   // 激励视频剩下可用的次数
    private popup_sprit: Popup;
    private current_block_ads: boolean = false;
    private ads: egret.Bitmap;
    ...
    // video的广告形式和选择
    private video_adver_name:Array<string> = ["iphoneX.mp4","ofo.mp4","kfc1.mp4","pinduoduo.mp4","nike.mp4","starbucks.mp4"];
    private video_select:Array<string>;
    private video_index:Array<number>;
    ...
    public constructor() {
        ...
        this.sequenceList = SequenceRandom.randomSequence();  // 生成本次游戏的游戏类型序列
    ...
    private selectBgPopAdver(){
        ...
        for(let x of this.video_select){
    ...
    private addPopupAdsFlagToBlock(){
        // 首先判断是否需要加block ads
        if(SceneMange.gameSceneSetting[this.sequenceList[Math.floor(this.sequence_index/6)][this.sequence_index%6]].popUp){
    ...
    private createKaijuShuoMingAddtishi(){
        if(this.sequence_index > 11){
            ...
            var progere: TransferScreen = new TransferScreen("第 3/3 部分 - 基础问题", "回答一些 基本问题", "quesScreen");
            ...
            var instrustion: string = "开始第 " + (Math.floor(this.sequence_index / 6) + 1).toString() + " 组 第 " + (this.sequence_index % 6 + 1).toString() + " 局游戏";
    ...
    private createKaijuShuoMing(){
        if(this.sequence_index > 11){
            ...
            var progere: TransferScreen = new TransferScreen("基础问题", "回答一些 基本问题", "quesScreen");
            this.addChild(progere);
        }else {
            this.removeChildren();
            this.createQuesBackGroundImage();
            ...
            var instrustion: string = "开始第 " + (Math.floor(this.sequence_index / 6) + 1).toString() + "/2 组" + (this.sequence_index % 6 + 1).toString() + "/6 局游戏";
            var insru: egret.TextField = new egret.TextField();
            insru.text = instrustion;
            insru.width = this.stageW;
            insru.textColor = 0x000000;
